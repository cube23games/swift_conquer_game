...
def main() -> int:
    ...
    analysis_gate = ROOT / "tools" / "sc211_analyze_gate.py"
    analysis_baseline = ROOT / "tools" / "sc211_analysis_baseline.json"

    for path in [project_path, shadow_workflow, production_workflow, agents, knowledge, analysis_gate, analysis_baseline]:
    ...
    require(
        project.get("validation_commands")
        == [["python3", "tools/sc211_analyze_gate.py"], ["flutter", "test"]],
        "baseline-aware validation command mismatch",
    )
    gate_config = project.get("analysis_gate") or {}
    require(gate_config.get("mode") == "locked-baseline-delta", "analysis gate mode mismatch")
    require(gate_config.get("baseline_issue_count") == 69, "analysis baseline count mismatch")
    baseline_data = json.loads(analysis_baseline.read_text(encoding="utf-8"))
    require(baseline_data.get("issue_count") == 69, "analysis baseline file count mismatch")
    require(len(baseline_data.get("issues", [])) == 69, "analysis baseline issue list mismatch")
    ...
    gate_text = analysis_gate.read_text(encoding="utf-8")
    for token in ["locked SC-211 baseline", "unexpected_keys", "--self-test"]:
        require(token in gate_text, f"analysis gate missing: {token}")
