...
## SwiftConquer project lock

- SwiftConquer is temporarily operated in `cube23games/swift_conquer_game`.
- The approved production branch for SC-211 onward is `working/phase170-movement-ci` until an explicit later promotion.
- The approved SC-211 baseline is `04d6db6d43a068612f1e79fc388666fdc75dee9b`.
- Flame renders snapshots and submits commands only; the deterministic headless simulation owns gameplay state.
- Preserve the Phase 171–210 battlefield, buildings, controls, production UI, camera bookmarks, and formation work while reconciling newer architecture.
- Do not invent unresolved faction, commander, naval, technology, economy, or superweapon canon.
- Use checksum-locked ZIP stages, Shadow CI first, then one focused production commit and exact CI run at a time.
- Local Flutter or Android application builds are forbidden; Android builds run in GitHub Actions.
